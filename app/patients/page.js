...
export default function Patients() {
  ...
  useEffect(() => {
    ...
    if (user?.role === "admin_institutions" && user?.institutionId) {
      patientsToShow = mockPatients.filter((patient) => patient.institutionIds.includes(user.institutionId))
    }
  ...
  const handleSaveEdit = (updatedPatient) => {
    const updatedPatients = patients.map((pat) => (pat.patientId === updatedPatient.patientId ? updatedPatient : pat))
  ...
  const handleSaveCreate = (newPatient) => {
    const patientWithId = {
      ...
      institutionIds: user?.role === "admin_institutions" ? [user.institutionId] : newPatient.institutionIds || [],
    }
  ...
  const confirmDelete = () => {
    if (patientToDelete) {
      const updatedPatients = patients.filter((pat) => pat.patientId !== patientToDelete.patientId)
      setPatients(updatedPatients)
  ...
  return (
    <DashboardLayout>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredPatients.map((patient) => (
                  <tr
                    ...
                    onClick={() => router.push(`/patients/${patient.patientId}/records`)}
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ...
                      <div className="text-gray-500">{patient.contact.email}</div>
                    </td>
      ...
      {showEditModal && selectedPatient && (
        <EditPatientModal patient={selectedPatient} onClose={() => setShowEditModal(false)} onSave={handleSaveEdit} />
      )}

      {showCreateModal && <CreatePatientModal onClose={() => setShowCreateModal(false)} onSave={handleSaveCreate} />}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900">{patient.contact.phone}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">Email</label>
              <p className="mt-1 text-sm text-gray-900">{patient.contact.email}</p>
            </div>
            <div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-900">{patient.contact.address}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Policy: {patient.insuranceInfo.policyNumber}</p>
          </div>
        </div>
